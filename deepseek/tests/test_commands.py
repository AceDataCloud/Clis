...
from deepseek_cli.main import cli
...
class TestGlobalCommands:
    ...
    def test_version(self, runner):
        ...
        assert "deepseek-cli" in result.output
...
class TestChatCommands:
    ...
    def test_chat_json(self, runner, mock_chat_response):
        respx.post("https://api.acedata.cloud/deepseek/chat/completions").mock(
    ...
    def test_chat_with_model(self, runner, mock_chat_response):
        route = respx.post("https://api.acedata.cloud/deepseek/chat/completions").mock(
            return_value=Response(200, json=mock_chat_response)
        )
        result = runner.invoke(
            cli,
            ["--token", "test-token", "chat", "Hello", "-m", "deepseek-r1", "--json"],
        )
        assert result.exit_code == 0

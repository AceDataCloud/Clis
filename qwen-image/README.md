...
[![PyPI version](https://img.shields.io/pypi/v/qwen-image-cli.svg)](https://pypi.org/project/qwen-image-cli/)
[![PyPI downloads](https://img.shields.io/pypi/dm/qwen-image-cli.svg)](https://pypi.org/project/qwen-image-cli/)
...
1. Sign up or log in
2. Navigate to the QwenImage API page
...
```bash
# Install with pip
pip install qwen-image-cli

# Or with uv (recommended)
uv pip install qwen-image-cli

# Or from source
git clone https://github.com/AceDataCloud/QwenImageCli.git
cd QwenImageCli
pip install -e .
```
...
# Check task status
qwen-image task <task-id>

# Wait for completion with polling
qwen-image wait <task-id> --interval 5
...
| Command | Description |
|---------|-------------|
| `qwen-image generate <prompt>` | Generate an image from a text prompt |
| `qwen-image edit <prompt> -i <url>...` | Edit or combine images using AI |

### Task Management

| Command | Description |
|---------|-------------|
| `qwen-image task <task_id>` | Query a single task status |
| `qwen-image tasks <id1> <id2>...` | Query multiple tasks at once |
| `qwen-image wait <task_id>` | Wait for task completion with polling |
...
TASK_ID=$(qwen-image generate "a red circle" --json | jq -r '.task_id')

# Wait for completion and get image URL
qwen-image wait $TASK_ID --json | jq -r '.data[0].image_url'

# Batch generate from a file of prompts
while IFS= read -r prompt; do
  qwen-image generate "$prompt" --json >> results.jsonl
done < prompts.txt
```
...
```bash
# Pull the image
docker pull ghcr.io/acedatacloud/qwen-image-cli:latest

# Run a command
docker run --rm -e ACEDATACLOUD_API_TOKEN=your_token \
  ghcr.io/acedatacloud/qwen-image-cli generate "A beautiful sunset"

# Or use docker-compose
docker compose run --rm qwen-image-cli generate "A beautiful sunset"
```
...
```
QwenImageCli/
├── qwen_image_cli/            # Main package
│   ├── __init__.py
│   ├── __main__.py            # python -m qwen_image_cli entry point
│   ├── main.py                # CLI entry point
│   ├── core/                  # Core modules
│   │   ├── client.py          # HTTP client for QwenImage API
...
## QwenImage CLI vs MCP QwenImage

| Feature | QwenImage CLI | MCP QwenImage |
|---------|----------------|----------------|
...
| Install | `pip install qwen-image-cli` | `pip install mcp-qwen-image-pro` |

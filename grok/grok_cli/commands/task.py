...
from grok_cli.core.output import print_error, print_json, print_success, print_task_result
...
def task(
    ...
    TASK_ID is the task ID returned from video generation commands.
...
def tasks_batch(
    ...
    \b
    Examples:
      grok tasks abc123 def456 ghi789
    """
    client = get_client(ctx.obj.get("token"))
    try:
...
def wait(
    ...
    Examples:
      ...
      grok wait abc123 --interval 10 --timeout 300
    """
    client = get_client(ctx.obj.get("token"))
    elapsed = 0

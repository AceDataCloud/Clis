...
def print_messages_result(data: dict[str, Any]) -> None:
    """Print a Claude Messages API result."""
    content_list = data.get("content", [])
    if content_list:
        for i, block in enumerate(content_list, 1):
            ...
            if block_type == "text":
                ...
                console.print(
                    Panel(
                        ...
                        title=title,
                        subtitle=f"[dim]{role} · {stop_reason}[/dim]"
                        if stop_reason
                        else f"[dim]{role}[/dim]",
                        border_style="green",
                    )
...
def print_count_tokens_result(data: dict[str, Any]) -> None:
    ...
    if input_tokens is not None:
        ...
        console.print("[yellow]No token count returned.[/yellow]")

...
@click.option(
    "-m",
    "--model",
    type=click.Choice(MINIMAX_MODELS),
    default=DEFAULT_MODEL,
    help="MiniMax model to use (default: MiniMax-H3).",
)
@click.option(
    "--image-url",
    "image_urls",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Return immediately with a task ID instead of waiting for the video.",
)
...
@click.option(
    ...
    type=click.Choice(MINIMAX_MODELS),
    default=DEFAULT_MODEL,
    help="MiniMax image-to-video model (default: MiniMax-H3).",
)
@click.option(
    "--ratio",
    type=click.Choice(RATIO_CHOICES),
